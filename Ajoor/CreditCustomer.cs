...
namespace Ajoor
{
    public partial class CreditCustomer : Form
    {
        ...
        private void bgwGetCustomers_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            {
                cmb_Customers.DataSource = _CustomerRepo.GetAllRecords().Where(p => p.CreatedBy == Utilities.USERNAME).Select(x => new { x.CustomerId, x.LastName, x.FirstName, x.AccountNumber, x.FullName }).ToList();
